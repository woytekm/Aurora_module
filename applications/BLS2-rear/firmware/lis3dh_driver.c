...
u8_t I2C_Mems_Read_Reg (u8_t Reg)
{
  ...
  // SEGGER_RTT_printf(0,"Reading reg:%X\n",Reg);

  err_code = nrf_drv_twi_tx(&m_twi_1, MEMS_I2C_ADDR, &reg_addr, sizeof(reg_addr),true);
  err_code = nrf_drv_twi_rx(&m_twi_1, MEMS_I2C_ADDR, &reg_data, sizeof(reg_data));

  // SEGGER_RTT_printf(0,"Reg %X, val: %X (%d)\n",Reg,reg_data,err_code);

  if(err_code != NRF_SUCCESS)
   SEGGER_RTT_printf(0,"MEMS ERROR Reg %X, val: %X (%d)\n",Reg,reg_data,err_code);

  APP_ERROR_CHECK(err_code);
  return reg_data;
}
 
void I2C_Mems_Write_Reg(u8_t WriteAddr, u8_t Data)
{
  ret_code_t err_code = 0;
  uint8_t packet[2] = {WriteAddr, Data};

  err_code = nrf_drv_twi_tx(&m_twi_1, MEMS_I2C_ADDR, packet, sizeof(packet),false);
  // SEGGER_RTT_printf(0,"I2C: wrote %X to reg %X (%d)\n",Data,WriteAddr,err_code);
...
u8_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data) {
  I2C_Mems_Write_Reg(WriteAddr, Data);  
  return 1;
}
...
/*******************************************************************************
* Function Name  : LIS3DH_ResetInt2Latch
